  ...
  config:
    providers:
      ...
      openai:
        ...
        requestImageMediaTypes: [image/png, image/jpeg] # stored types the route decodes
...
Supported profile fields are `apiKeyEnv`, `displayName`, `api`, `baseURL`, `models`, `modelOverrides`, `compat`, `defaultContextWindow`, `defaultMaxTokens`, `defaultInput`, `headers`, `reasoning`, `thinkingBudgets`, `cacheRetention`, `transport`, `timeoutMs`, `websocketConnectTimeoutMs`, `streamIdleTimeoutMs`, `maxRequestImageBytes`, `requestImagePixelBudget`, `requestImageMaxBytes`, `requestImageMediaTypes`, and `retryPolicy`. Each resolved profile retry policy is captured with that provider route; omission uses the shared bounded normal default of five retries. The stream-idle interval is a positive finite Node timer delay, defaults to five minutes, and covers only an outstanding provider read, not consumer think time. Every image route derives a deterministic request version from the provider-independent normalized attachment under `requestImagePixelBudget` (default 2048 by 2048 total pixels) and `requestImageMaxBytes` (default 1MiB raw bytes). `requestImageMediaTypes` restricts which stored media types pass through or are re-encoded into the request version; omit it or leave it empty to keep every stored media type. Declare the types the route's backend actually decodes — for example `[image/png, image/jpeg]` for a llama.cpp OpenAI-compatible server whose image decoder rejects WebP — so a stored WebP screenshot is re-encoded instead of failing the request. An alpha image a declared list cannot carry (for example a list without a transparent type) fails the request with `UNSUPPORTED_IMAGE_TYPE` for that route. Before reading attachments, `maxRequestImageBytes` applies to conservative request-version upper bounds and replaces the oldest over-budget images with fixed text; exact base64 lengths are checked again after retained versions are generated. The 20MiB default can retain fifteen maximum-size 1MiB versions after base64 expansion while leaving request-body headroom. The same version feeds inline base64, and its stable descriptor exposes the attachment id and actual request-image dimensions. Harness app attribution wins a conflicting configured header name.
