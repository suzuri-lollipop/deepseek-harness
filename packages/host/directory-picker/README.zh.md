...
- **不支持按部署限定浏览根**——`listRoots` 汇报平台自身的根（Windows 为存在的各盘符根，POSIX 为 `/`；不存在或不可访问的盘符因 stat 失败而被省略），无法把浏览树限制到配置的根子集。该限定仍等到出现消费方再做，见 DirectoryPicker Agent Note。
